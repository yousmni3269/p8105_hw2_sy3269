...
ada_stn = select(unique_stn, ada == TRUE)
```
...
```{r, station for A}
stn_for_A = distinct(new_metro_df, route_name == "A") 
...
prof_tw_df = read_excel(
  "./202309 Trash Wheel Collection Data.xlsx", sheet = "Professor Trash Wheel", 
  skip = 1, 
  n_max = 106, 
  na = c("NA", "", ".")) |>
  janitor::clean_names() |>
  ...
  select(dumpster:wrappers, sports_balls, homes_powered)
...
gwyn_tw_df = read_excel(
  ...
  select(dumpster:cigarette_butts, glass_bottles, plastic_bags, wrappers, sports_balls, homes_powered)

com_tw_df = bind_rows(prof_tw_df, gwyn_tw_df, mr_tw_df)
```
...
I need to add a summary of the data set. 
