...
From the merged data set, only the series, episode, baker name and result of the competition information was selected to create a new data frame of the Great British Bake Off winner (or star baker) of each episode in Season 5 to 10. To make the data set more human-readable, the data frame was reformatted using `pivot_longer` and organized using `knitr::kable`. 

From the table, it is surprising to see some of the bakers win multiple times in one season. For instance, Richard became the *star baker* five times in Season 5, and Steph was a four time *star baker* in Season 10. 
