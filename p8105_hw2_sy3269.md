...
gbboff_winner = 
  gbboff_df |>
  select(series, first_name, result_1:result_10) |>
  pivot_longer(
    cols = result_1:result_10, 
    names_to = "episode", 
    values_to = "result"
  ) |> 
  filter(result == "STAR BAKER", series > 4) |>
  mutate(
    episode = case_match(
      episode,
      "result_1" ~ 1,
...
| series | episode | first_name | result     |
|-------:|--------:|:-----------|:-----------|
...
From the merged data set, only the series, episode, baker name and
result of the competition information was selected to create a new data
frame of the Great British Bake Off winner (or star baker) of each
episode in Season 5 to 10. To make the data set more human-readable, the
data frame was reformatted using `pivot_longer` and organized using
`knitr::kable`.

From the table, it is surprising to see some of the bakers win multiple
times in one season. For instance, Richard became the *star baker* five
times in Season 5, and Steph was a four time *star baker* in Season 10.
